...
func main() {
	...
	flag.BoolVar(&compress, "gzip", false, "whether the item should be compressed during upload")
	...
	bucket = mustGetEnv("GCS_HEAP_DUMP_BUCKET", bucket)
	projectID = mustGetEnv("GCS_HEAP_DUMP_PROJECT_ID", projectID)
	...
	_, objAttrs, err := upload(ctx, r, projectID, bucket, name, public, compress)
	if err != nil {
		switch err {
...
func upload(ctx context.Context, r io.Reader, projectID, bucket, name string, public bool, compress bool) (*storage.ObjectHandle, *storage.ObjectAttrs, error) {
